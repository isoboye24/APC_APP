...
{
    public class MemberCommittmentRepository : IMemberCommittmentRepository
    {
        private readonly APCEntities _db;
        public MemberCommittmentRepository(APCEntities db)
        {
            _db = db;
        }
        ...
        public List<MemberCommittment> GetMembersCommittment(int year)
        {
            ...
            var members = _db.MEMBER
                .Where(x => !x.isDeleted
                    && x.membershipStatusID == 1
                    && x.membershipDate.Value.Year <= year)
                .Select(m => new
                {
                    m.memberID,
                    m.name,
